...
import { LifeBuoy, CheckCircle, Clock, AlertCircle, Bot, User } from 'lucide-react';
...
const AdminTickets = () => {
  ...
  const loadTickets = async () => {
    try {
      // Load customer service tickets
      const { data: csTickets } = await supabase
        .from('customer_service_tickets')
        .select('*')
        .order('created_at', { ascending: false });

      // Load AI support tickets
      const { data: aiTickets } = await supabase
        .from('ai_support_tickets')
        .select('*')
        .order('created_at', { ascending: false });

      // Normalize and combine tickets
      const normalizedCsTickets: Ticket[] = (csTickets || []).map(ticket => ({
        ...ticket,
        source: 'dashboard' as const,
      }));

      const normalizedAiTickets: Ticket[] = (aiTickets || []).map(ticket => ({
        ...ticket,
      ...
      // Fetch user names for all tickets
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-purple-600">{aiTickets.length}</div>
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs defaultValue="all" className="space-y-4">
        ...
        <TabsContent value="ai" className="space-y-4">
          {loading ? (
            ...
            <div className="text-center py-8 text-muted-foreground">No AI-created tickets</div>
          ) : (
            aiTickets.map(renderTicketCard)
          )}
        </TabsContent>
        ...
        <TabsContent value="dashboard" className="space-y-4">
          {loading ? (
            ...
            <div className="text-center py-8 text-muted-foreground">No dashboard tickets</div>
          ) : (
            dashboardTickets.map(renderTicketCard)
          )}
        </TabsContent>
